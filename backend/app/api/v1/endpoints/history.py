...
from app.services.image_service import ImageResultNotFoundError, InvalidImageResultError, image_service
...
def get_history_result(run_id: str) -> FileResponse:
    try:
        ...
        if run.result_file_id.startswith("image:"):
            ...
            return FileResponse(path=image_service.get_result_path(filename), media_type="image/png", filename=filename)
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    except (FileNotFoundError, ImageResultNotFoundError) as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
